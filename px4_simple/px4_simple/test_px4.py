...
class OffboardControl(Node):
    ...
    def __init__(self) -> None:
        ...
        self.offboard_control_mode_publisher = self.create_publisher(
            OffboardControlMode, '/spacer/fmu/in/offboard_control_mode', qos_profile)
        self.trajectory_setpoint_publisher = self.create_publisher(
            TrajectorySetpoint, '/spacer/fmu/in/trajectory_setpoint', qos_profile)
        self.vehicle_command_publisher = self.create_publisher(
            VehicleCommand, '/spacer/fmu/in/vehicle_command', qos_profile)
        self.publisher_direct_actuator = self.create_publisher(
            ActuatorMotors,
            "/spacer/fmu/in/actuator_motors",
            qos_profile,
        )

        # Create subscribers
        self.vehicle_local_position_subscriber = self.create_subscription(
            VehicleLocalPosition, '/spacer/fmu/out/vehicle_local_position', self.vehicle_local_position_callback, qos_profile)
        self.vehicle_status_subscriber = self.create_subscription(
            VehicleStatus, '/spacer/fmu/out/vehicle_status', self.vehicle_status_callback, qos_profile)

        # Initialize variables
        self.offboard_setpoint_counter = 0
        self.vehicle_local_position = VehicleLocalPosition()
        self.vehicle_status = VehicleStatus()
        self.get_logger().info(f"ini_veihcles status {self.vehicle_status.nav_state}")
    ...
    def publish_direct_actuator_setpoint(self, u_command):
        ...
        self.get_logger().info(f"Publishing thrust: {actuator_outputs_msg}")
